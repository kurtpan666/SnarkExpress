    ...
    <div className="max-w-7xl mx-auto px-2 sm:px-4 py-4 sm:py-6">
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
        ...
        <div className="lg:w-56 lg:flex-shrink-0">
          <div
            className={`
              ...
              bg-gray-50 dark:bg-gray-800 lg:bg-transparent lg:dark:bg-transparent
              p-4 lg:p-0
          ...
          >
            ...
            <button
              onClick={() => setIsSidebarOpen(false)}
              className="lg:hidden absolute top-2 right-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 z-10"
            ...
            <div className="space-y-4">
              ...
              {/* Tag Filter */}
              <div className="bg-white dark:bg-gray-800 rounded border border-gray-300 dark:border-gray-600 shadow-sm">
                <button
                ...
                >
                  <h3 className="font-bold text-sm text-gray-900 dark:text-white">Filter by tag:</h3>
                  <svg
                ...
                {!isTagsCollapsed && (
                  <div className="px-3 pb-3 space-y-1 text-sm max-h-64 overflow-y-auto">
                    <button
                      ...
                      className={`block w-full text-left px-2 py-1 rounded transition-colors text-gray-900 dark:text-gray-100 ${
                        !selectedTag ? 'bg-orange-100 dark:bg-orange-900 font-semibold' : 'hover:bg-gray-100 dark:hover:bg-gray-700'
                      }`}
                    >
                      All
                    </button>
                    {tags.map((tag) => (
                      <button
                        ...
                        className={`block w-full text-left px-2 py-1 rounded transition-colors text-gray-900 dark:text-gray-100 ${
                          selectedTag === tag.name ? 'bg-orange-100 dark:bg-orange-900 font-semibold' : 'hover:bg-gray-100 dark:hover:bg-gray-700'
                        }`}
                      >
                        {tag.name} ({tag.count})
                      </button>
                  ...
                  {!isRecsCollapsed && (
                    <div className="px-3 pb-3 space-y-3 text-sm max-h-96 overflow-y-auto">
                      {recommendedPapers.map((paper) => (
                        <div key={paper.id} className="border-b border-gray-100 dark:border-gray-700 pb-2 last:border-b-0">
                          <a
                            href={paper.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-gray-900 dark:text-gray-100 hover:text-orange-600 dark:hover:text-orange-400 line-clamp-2 block transition-colors"
                            title={paper.title}
                          >
                            {paper.title}
                          </a>
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white dark:bg-gray-800 rounded border border-gray-300 dark:border-gray-600 shadow-sm">
            {loading ? (
              <div className="p-4 text-center text-gray-500 dark:text-gray-400">Loading papers...</div>
            ) : papers.length === 0 ? (
